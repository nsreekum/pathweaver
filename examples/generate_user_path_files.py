...
try:
    ...
    from pathweaver.simulation.recorder import record_user_simulation_data
...
USER_SPEED = 1.5  # Speed in units per second (e.g., meters/sec)
FOV_RADIUS = 15.0 # Radius for Field of View calculation (in map units)
OUTPUT_CSV_DIR = "simulation_output" # Directory to save CSV files
COORDINATE_SYSTEM = 'xy' # Use 'xz' if Z is up/down on plot, 'xy' otherwise
...
if __name__ == "__main__":
    ...
    try:
        ...
        # --- Step 6: Record Simulation Data (NEW STEP) ---
        print(f"\n[7/N] Recording simulation data to CSV files in '{OUTPUT_CSV_DIR}'...")
        if not os.path.exists(OUTPUT_CSV_DIR):
            print(f"  Creating output directory: {OUTPUT_CSV_DIR}")
            os.makedirs(OUTPUT_CSV_DIR)

        for user_data in shopping_paths_data:
            record_user_simulation_data(
                user_path_data=user_data,
                item_placements=item_placements,
                user_speed=USER_SPEED,
                fov_radius=FOV_RADIUS,
                output_dir=OUTPUT_CSV_DIR,
                coordinate_system=COORDINATE_SYSTEM # Pass coord system info
            )
